...
const modelValue = defineModel<ProgrammedIncome>();

const emit = defineEmits<{
  submit: [income: ProgrammedIncome];
}>();
...
function onSelectFrequency(item: Item) {
  ...
  // A month only applies to yearly incomes. Drop any previously picked
...
function onSubmit(values: ProgrammedIncome) {
  ...
  <AppCard>
    <AppCardBody>
      <p class="font-serif text-2xl font-bold text-text-1">
        {{
          formMode === "insert"
            ? "New Programmed Income"
            : "Edit Programmed Income"
        }}
      </p>
    </AppCardBody>
    <Form
