...
export interface ProgrammedIncomeInput {
  frequency?: unknown;
  chargeDay?: unknown;
  chargeMonth?: unknown;
  [key: string]: unknown;
...
// Mirrors the programmed_incomes CHECK constraints so invalid charge
// schedules fail with a 400 instead of a 500 from Postgres. A month only
// applies to yearly incomes and is forced to null otherwise.
export function validateChargeSchedule(
  body: ProgrammedIncomeInput,
): ScheduleValidation {
  ...
  if (frequency === Frequency.YEARLY) {
    ...
    if (month === null || month < 1 || month > 12) {
      return {
        ok: false,
        message:
          "chargeMonth must be an integer between 1 and 12 for yearly incomes",
      };
    }
